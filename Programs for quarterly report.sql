SELECT        
    ...
    HIFIS_Programs.ProgramDateStart AS "PROG START DATE",
    HIFIS_Programs.ProgramDateEnd AS "PROG END DATE",
  ...
  WHERE
    ...
    --Choosing just cluster 1
    AND HIFIS_Organizations.OrganizationID <> 1
    --Removing MSP's access points for exit up
    AND NOT (HIFIS_Programs.ProgramID = 32 AND HIFIS_Organizations.OrganizationID in (13,14,15,17,19,20))
    --Removing MSP Emergency for SBSL
    AND NOT (HIFIS_Programs.ProgramID =9 AND HIFIS_Organizations.OrganizationID=12)
    --Removing TLW and RAY for Reaching Home
    AND NOT (HIFIS_Programs.ProgramID =33 AND HIFIS_Organizations.OrganizationID in (61,62))
